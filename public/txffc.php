...
$json = httpGet('http://77tj.org/api/tencent/onlineim');

if(strrpos($json, 'onlinenumber') !== false){
	echo $json;
}else{
	$json = httpGet('http://77tj.org/api/tencent/onlineim');
	if(strrpos($json, 'wscckey') !== false){
		echo httpGet(get_between($json, "window.location.href='", "';</script>"));
	}else{
		echo $json;
	}
}
